...
resources - https://stackoverflow.com/questions/8856815/how-to-return-the-k-th-element-in-treeset-in-java


## Helps with order statistics tree

typically done with Balanced BST augmentation.
Structural induction on the size of the sub-tree will give you rank.

e.g. rank would be a node's position in an inorder traversal of BST.


## rank(node)

These structures are typically implemented as balanced binary search trees that store information in each node counting how many elements are to the left or right of the node, so a search down the tree can be made to find the appropriate element by descending into the appropriate subtree at each step

### Sample implementation (augmenting RBTree)
